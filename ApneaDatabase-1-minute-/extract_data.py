...
# extract edf file content without adding noise
def edf_to_csv(file, timestamp_rate = 60, fragment_n = 0):

    f = pyedflib.EdfReader(file)

    temp = f.readSignal(11)[5::timestamp_rate*100]
    
    spo2 = f.readSignal(15)[5::timestamp_rate]
    body = f.readSignal(16)[5::timestamp_rate]
    #movimiento = np.select([body < 1.5, body < 4.5], ['Up', 'Right'], 'Left')
    movimiento = np.select([body < 1.5, body < 4.5], ['3', '1'], '2')
    hr = f.readSignal(17)[5::timestamp_rate]

    duration = len(hr)

    timestamp = np.arange(duration*timestamp_rate*fragment_n, duration*timestamp_rate*(fragment_n+1), 60)

    sound = f.readSignal(19)
...
for folder in folders:
    ...
    for file in files:
        ...
        data, sound = edf_to_csv(path, timestamp_rate=timestamp_rate, fragment_n=file_count-1)
